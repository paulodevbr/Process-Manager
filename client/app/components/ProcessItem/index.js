...
function ProcessItem({process, login, onDelete}) {
  ...
  return (
    <div>
      ...
      <Row>
        <Col md={10} xs={8}><ProcessCreationText>{process.dateOfCreation}</ProcessCreationText><CustomBadge/></Col>
        <Col md={2} xs={4}>
          <ButtonGroup>
            <Button variant="light"><i className="fas fa-edit" style={{color: "dark"}}/></Button>
            <Button variant="light" onClick={onDelete(process.id)} disabled><i className="fas fa-trash" style={{color: "red"}}/></Button>
          </ButtonGroup></Col>
      </Row>
    </div>
  );
