...
_Step 3:_ Follow the standard Heroku deploy legalProcess:
...
```diff
+ const BUILD_FOLDER_PATH = legalProcess.env.BUILD_FOLDER_PATH || 'build';
+ const PUBLIC_PATH = legalProcess.env.PUBLIC_PATH || '/';
```

```diff
- path: path.resolve(legalProcess.cwd(), 'build'),
- publicPath: '/',
+ path: path.resolve(legalProcess.cwd(), BUILD_FOLDER_PATH),
...
+ const basename = legalProcess.env.PUBLIC_PATH;
